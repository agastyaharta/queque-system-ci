...
WHERE registration.`status` = 'Valid' AND registration.`employeeid` IS NULL
...
if (!empty($requestData['search']['value'])) {
	...
	$sql = "SELECT registration.`registrationid`, patient.`patientname`
        FROM registration
        INNER JOIN patient ON patient.`patientid` = registration.`patientid`
        WHERE registration.`status` = 'Valid' AND registration.`employeeid` IS NULL AND  registration.`registrationid` like '" . $requestData['search']['value'] . "%' ";
